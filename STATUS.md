...
|---|---|---|---|---|---|---|
| In review | [#347](https://github.com/taihartman/obsidian-atoms/issues/347) [#348](https://github.com/taihartman/obsidian-atoms/issues/348) | taihartman | `claude/settings-form-rows-back-chevron` | [plan](docs/plans/2026-08-06-004-fix-settings-form-rows-and-back-chevron-plan.md) · [QA](docs/qa/2026-08-06-347-348-settings-form-rows-world-class-qa.md) | `src/settings/rows.ts`, `src/settings/settings.ts`, `styles.css`, `test/settingsRows.test.ts`, `test/settings.test.ts`, `docs/qa/app-navigation-map.md` | **PR #349 out of draft; full tail ran** — doc-review, simplify, code-review, compound, world-class-qa + adversarial. Adversarial found and fixed an iPad squeeze (`is-tablet` gets neither Obsidian's stacking nor the `is-phone` rule). **CI green on both checks; #345 merged, so this is rebased onto `master` and no longer stacked.** Follow-ups filed: #350, #351. A field and the one button that commits it become one `formRow` — a sixth row kind, **composed not configured**; four pairs, 8 rows → 4; `accountInput` and the three `dataset.plus*` markers delete. Back chevron moves to the row's leading edge in the **base** `.atoms-setting-back` rule — desktop has the same 236px gutter, so this is not an `is-phone` fix · plugin bump |
