...
|---|---|---|---|---|---|---|
| In review | [#347](https://github.com/taihartman/obsidian-atoms/issues/347) [#348](https://github.com/taihartman/obsidian-atoms/issues/348) | taihartman | `claude/settings-form-rows-back-chevron` | [plan](docs/plans/2026-08-06-004-fix-settings-form-rows-and-back-chevron-plan.md) · [QA](docs/qa/2026-08-06-347-348-settings-form-rows-world-class-qa.md) | `src/settings/rows.ts`, `src/settings/settings.ts`, `styles.css`, `test/settingsRows.test.ts`, `test/settings.test.ts`, `docs/qa/app-navigation-map.md` | **PR #349 out of draft; full tail ran** — doc-review, simplify, code-review, compound, world-class-qa + adversarial. Adversarial found and fixed an iPad squeeze (`is-tablet` gets neither Obsidian's stacking nor the `is-phone` rule). **Blocked only on #345's CI**, which creates no runs at all — repo-level, needs the owner. Follow-ups filed: #350, #351. **Stacked on `fix/342-proposed-tags-dismiss-row`**, which claims the same four files — #345 is finished but GitHub has fired no CI run for any commit after `a0c7665`, so this branch sits on its head rather than idling. Rebase onto `master` the moment #345 merges. A field and the one button that commits it become one `formRow` — a sixth row kind, **composed not configured**; four pairs, 8 rows → 4; `accountInput` and the three `dataset.plus*` markers delete. Back chevron moves to the row's leading edge in the **base** `.atoms-setting-back` rule — desktop has the same 236px gutter, so this is not an `is-phone` fix · plugin bump |
| In progress | [#342](https://github.com/taihartman/obsidian-atoms/issues/342) [#346](https://github.com/taihartman/obsidian-atoms/issues/346) | taihartman | `fix/342-proposed-tags-dismiss-row` | amend lane ([handoff](docs/handoffs/2026-08-06-proposed-tags-dismiss-row.md)) | `src/settings/settings.ts`, `src/settings/rows.ts`, `styles.css`, `test/helpers/settingsTab.ts`, `docs/qa/app-navigation-map.md` | Each proposed tag rendered a second full row whose only content was its own Dismiss button. Per-tag Dismiss goes away; one section-level **N proposals waiting / Dismiss all** row takes its place, scoped to the proposals it rendered so a concurrent classify merge is not wiped. Row grammar is **not** weakened — no two-action primitive. Rides #346: on a **phone** Obsidian stacked every row whose right edge is not a toggle, so destination / back / status rows orphaned their chevron or trailing text onto a second line — the gate is `is-phone`, and `emulateMobile` at desktop width does **not** reproduce it · plugin bump |
