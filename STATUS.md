...
|---|---|---|---|---|---|---|
| In review | [#427](https://github.com/taihartman/obsidian-atoms/issues/427) / [PR #428](https://github.com/taihartman/obsidian-atoms/pull/428) | taihartman (agent) | `claude/backfill-opt-in-709c23` | [plan](docs/plans/2026-08-10-003-feat-auto-filing-window-backfill-split-plan.md) | `src/platform/autorun.ts` · `src/plugin/main.ts` · `src/pipeline/{parse,daily,write}.ts` · `src/settings/settings.ts` · `src/home/{atomsHomeData,atomsHomeView}.ts` | Automatic filing bounded to a window starting the enable day; history moves behind the priced backfill offer. **Launch-blocking subset only: U1–U4 + U6.** U5/U7/U8 deferred (U8 needs its own doc-review). Does **not** touch `EGRESS_DISCLOSURE` or bump `EGRESS_ACK_VERSION` — narrowing unattended reach never needs re-consent. **KTD1 reversed by the owner:** the enable tap no longer files today, because clause (3) of the disclosure says today is never auto-touched and rewording it would strand every device's consent — so day one is deliberately silent. Shipping tail complete (simplify · code-review incl. cross-model adversarial on Grok · compound · world-class-qa + adversarial-qa, [report](docs/qa/2026-08-10-427-auto-filing-window-world-class-qa.md)). Follow-ups filed: #429, #430, #431. **No version bump yet** — that is U7. |
