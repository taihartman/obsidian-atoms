...
|---|---|---|---|---|---|---|
| In progress | [#347](https://github.com/taihartman/obsidian-atoms/issues/347) [#348](https://github.com/taihartman/obsidian-atoms/issues/348) | taihartman | `claude/settings-form-rows-back-chevron` | [plan](docs/plans/2026-08-06-004-fix-settings-form-rows-and-back-chevron-plan.md) | `src/settings/rows.ts`, `src/settings/settings.ts`, `styles.css`, `test/settingsRows.test.ts`, `test/settings.test.ts`, `docs/qa/app-navigation-map.md` | **Stacked on `fix/342-proposed-tags-dismiss-row`**, which claims the same four files — #345 is finished but GitHub has fired no CI run for any commit after `a0c7665`, so this branch sits on its head rather than idling. Rebase onto `master` the moment #345 merges. A field and the one button that commits it become one `formRow` — a sixth row kind, **composed not configured**; four pairs, 8 rows → 4; `accountInput` and the three `dataset.plus*` markers delete. Back chevron moves to the row's leading edge in the **base** `.atoms-setting-back` rule — desktop has the same 236px gutter, so this is not an `is-phone` fix · plugin bump |
