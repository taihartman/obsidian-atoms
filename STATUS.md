...
|---|---|---|---|---|---|---|
| In progress | [#342](https://github.com/taihartman/obsidian-atoms/issues/342) | taihartman | `fix/342-proposed-tags-dismiss-row` | amend lane ([handoff](docs/handoffs/2026-08-06-proposed-tags-dismiss-row.md)) | `src/settings/settings.ts`, `src/pipeline/vocabulary.ts` | Each proposed tag rendered a second full row whose only content was its own Dismiss button. Per-tag Dismiss goes away; one section-level **Dismiss all proposed** row takes its place. Row grammar is **not** weakened — no two-action primitive · plugin bump |
