...
describe("#360 askAckIsCurrent", () => {
  ...
  it("reads each ack against its own shipped version, never the other's", () => {
    expect(
      askPrivacyAckIsCurrent({ askPrivacyAckVersion: ASK_WRITE_ACK_VERSION }),
    ).toBe(ASK_WRITE_ACK_VERSION === ASK_PRIVACY_ACK_VERSION);
    expect(askPrivacyAckIsCurrent({ askPrivacyAckVersion: ASK_PRIVACY_ACK_VERSION })).toBe(true);
    expect(askWriteAckIsCurrent({ askWriteAckVersion: ASK_WRITE_ACK_VERSION })).toBe(true);
  });
