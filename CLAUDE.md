...
5. **PR** — body must include **`Closes #<issue>`** (auto-close the hard-claim Issue; “Issue #N” alone is not enough) + distilled **Core user stories** + **Edge cases & testing** (link full report under `docs/qa/YYYY-MM-DD-*-world-class-qa.md`)
  6. **PR evidence (non-negotiable):** mark every **Test plan** checkbox only after it ran; for **UI / product-facing** changes, **attach** screenshots (or a short clip) of the live vault smoke to the PR — `gh pr create --attach 'shot.png#Home after Process'` (repeatable; needs `gh` ≥ 2.99.0; `gh pr edit` and `gh pr comment` take the same flag). Reference the file relatively in the body — `![Home after Process](./shot.png)` — and gh rewrites it to the uploaded asset, so images land in the Evidence table instead of appended at the end. Capture via `obsidian vault="test vault" dev:screenshot path=…` after `./scripts/install-to-vault.sh`. Review screenshots are **not committed** — attachments live on GitHub's asset CDN, so they stay out of git history and can't break when the branch is deleted. `docs/qa/screenshots/<branch-or-feature>/` is reserved for **baselines** a later session must re-locate; those stay committed and are linked with **absolute** `https://raw.githubusercontent.com/<owner>/<repo>/master/…` URLs pinned to `master` or a sha, never a feature branch (PR descriptions do **not** render repo-relative image paths — they show a broken icon). Docs-only / pure logic: write `N/A — no UI` instead of fake images. Leaving the template boxes unchecked after “done” is a shipping-tail bug.
...
**Done means:** simplify + code-review + compound + world-class-qa (incl. adversarial half) ran, or an explicit, recorded skip for pure mechanical churn (renames, version-only bumps with no logic, docs-only); PR Test plan boxes match real evidence; UI PRs have vault screenshots attached to the PR.  
