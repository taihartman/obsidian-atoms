...
- **Search expansion (optional):** when `ASK_EXPAND_ENABLED=1` (**off by default** — this is the only Ask path that sends body plaintext off-device, and the Ask privacy ack carries no version to invalidate older grants) and `ANTHROPIC_API_KEY` is set, Plus may send title/tags/body slices to Anthropic on mirror upsert to build encrypted expansion phrases for `search_atoms`. Leaving it unset keeps pure lexical search. Expansion text is never returned on MCP payloads.
