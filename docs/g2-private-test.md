...
Private `0.1.19` moves companion HTTP requests from WKWebView `fetch` to its native `XMLHttpRequest` path after physical iPhone evidence showed the request never reached the live redeem boundary. DPoP proof, nonce, code, Origin, CORS, timeout, and fail-closed server enforcement are unchanged. The packaged artifact is `46,659,786` bytes with SHA-256 `ddf4f98dce326010a40ec85ebcbd68c2dcb98701b090330a45216579bf4141ba`.
