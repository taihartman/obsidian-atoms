...
Automated evidence: 20 companion Vitest files and 166 tests pass; the companion production build and package verification pass. All 720 Plus service tests pass. All 2,380 Obsidian tests pass. Plugin production build and lint pass. The security contract validates, including atomic current-authorization enqueue, confirmed-only payloads, tenant-scoped claim, durable marker-backed acknowledgment after user edits, ciphertext purge, and seven-day receipt reduction to a tombstone.

Production release 76 runs image `atoms-plus:deployment-01M269NGRMK8R9GB547220ZKKX`. The relay table was created under the dark release first. After health and schema verification, relay-only encryption, DPoP nonce, retention, and dynamic iPhone loopback-origin secrets were configured and G2 was enabled. `/health` passed, a canonical loopback request reached the live G2 auth boundary with 401, a wrong Origin returned 403, and unauthenticated MCP POST remained 401. Private `0.1.15` was uploaded to Even Hub on 2026-09-10; `0.1.14` is superseded because it did not surface the required re-pair for older tokens.
