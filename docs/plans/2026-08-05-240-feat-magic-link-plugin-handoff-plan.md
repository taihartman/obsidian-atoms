...
**Patterns to follow:** the `sessions.verified` back-fill at `plus-service/src/store/sqlite.mjs:96-100` and the additive ALTER block at `postgres.mjs:107-112`. `plus-service/src/store/shared.mjs:16` already exposes a sha256 hex helper — reuse it rather than adding a second hashing path, for both `verifier_hash` and the token key.
...
6. Factor the hash-compare so U13's peek endpoint applies exactly the same check — one comparison, two callers, so a peek can never say usable where an exchange would refuse.
...
- The S256 challenge of a known verifier matches a known digest, computed by the real `crypto.subtle`.
