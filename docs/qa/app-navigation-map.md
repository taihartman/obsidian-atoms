...
- **Plus service URL override:** Advanced destination. Point it at a local `plus-service` (`http://127.0.0.1:8787`) to drive Ask mirror QA without a cloud account. Reachable in the UI; `obsidian eval` is no longer required. **Validated since #500** (`isAllowedPlusBaseUrl`, `src/platform/plusClient.ts`): empty = hosted default, `https://` on any host, `http://` **only** on loopback (`localhost` / `127.x.x.x` / `[::1]`), everything else refused. A refused value still saves — the field persists per keystroke — and prints `PLUS_BASE_URL_INVALID_MESSAGE` in a `.atoms-setting-error` div under the row (`:empty` hides it, so a cleared error leaves no gap). Refused bases fail every Plus call rather than falling back to production.
