...
| Group | Header | Rows |
|---|---|---|
| Status | `Get started` (nobody files yet) / `Status` (somebody does) | Unconfigured: **one** destination row named after the next setup step, subtitle `Required` — `Turn on Daily Notes` (→ Obsidian's core-plugins tab) or `Choose who files your captures` (→ **engine**). Filing: `File automatically` + `Next run` status row |
...
| Leg 2 | `2 · File` | `Who does the filing` (→ engine) · `File automatically when Obsidian opens` **only while setup is unfinished** · `Atom folder` · `Tag vocabulary · N active` (→ vocabulary) · `List atoms on hub notes` · `Refresh hub lists` (hub lists on) |
...
|---|---|---|
| **Engine** (0.8.0) | `Who does the filing` in the File group, and the `Choose who files your captures` setup step | `Set up automatic filing` (the account entry — `Set up automatic filing` signed out, name varies by `accountRowDescriptor` state), `Anthropic API key`, `Device-local key fallback`, then `What gets sent` as three egress facts. The naming rule for the secret id lives in the `Pick one` group **footer**, under both key rows, not in the key row itself. |
...
- **Self-host Ask:** that name stays off the main screen. The guide is `docs/ask-self-host.md`. Advanced carries **Self-host guide** (Open) next to `Plus service URL` (renamed from `DIY Ask guide` / `Plus service URL override` in 0.8.0).
