...
|---|---|---|
| **Engine** (0.8.0) | `Filing` in the File group (renamed from `Who does the filing` in 0.8.1 — `DESTINATION_TITLES.engine`, `settings.ts:902`), and the `Choose who files your captures` setup step | `Atoms Plus` (the account entry — named `Atoms Plus` in **every** state since 0.8.1; the state label from `accountRowDescriptor` rides in the row's *description*, not its name, and signed out the description carries the offer instead), `Anthropic API key`, `Device-local key fallback`, then `What gets sent` as three egress facts. The naming rule for the secret id lives in the `Pick one` group **footer**, under both key rows, not in the key row itself. |
